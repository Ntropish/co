...
export const from = curry((range: range, value) => (value - range[0]) / duration(range))

export const to = curry((range: range, value) => value * duration(range) + range[0])

export const line = curry((a: range, b: range, value) => to(b, from(a, value)))

export const pow = curry((power, a, b, value) => to(b, from(a, value) ** power))

export const toFixedString = (range: range) => range.map(n => n.toFixed(2))

export const duration = ([start, end]: range) => end - start
export { duration as width }

export const grow = curry((amount, origin, range: range) => {
  return add(range, [-amount * origin, amount * (1 - origin)])
})

export const scale = curry((amount, origin, range: range) => {
  return grow(duration(range) * (amount - 1), origin, range)
})
...
export const containsRange = curry((container, range: range) => {
  return contains(container, range[0]) || contains(container, range[1]) || contains(range, container[0])
})

export const valueIn = curry((value, range: range) => contains(range, value))

export const clamp = curry((range: range, value) => {
...
export function curry(fn: (...args: any[]) => any) {
  ...
  function chef(args = [] as any[]) {
    if (args.length >= fn.length) {
      return fn(...args)
    } else {
      const argsGetter = function(...newArgs: any[]) {
        return chef(args.concat(newArgs))
      }
      Object.defineProperty(argsGetter, 'name', {
        value: `Give me ${fn.length - args.length} args`,
        writable: false,
      })
